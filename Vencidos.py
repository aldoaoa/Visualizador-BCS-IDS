                    ...
                    if es_duplicado:
                        ...
                        with st.spinner("Guardando en base de datos..."):
                            ...
                            nueva_fila = [
                                ...
                                fabricante_final,                                
                                float(nuevo_minimo),                             
                                float(limite_alta) if "E" in str(limite_alta).upper() else limite_alta, 
                                unidad_medida,                                          
                                val_guardar,      
                                unidad_medida,                                          
                                metodo,                                           
    ...
    # ==========================================
        ...
        if id_escaneado_url:
            ...
            if es_mob or es_ion:
                ...
                # --- NUEVA LÓGICA DE REACTIVACIÓN VISUAL ---
                if estatus_actual_op == "NO OPERATIVO":
                    ...
                    if hacer_medicion:
                        with st.form("form_actualizacion"):
                            ...
                            if st.form_submit_button(texto_boton_submit):
                                if (not es_ion and (base_upd is None or exp_upd is None)) or (es_ion and (v_act is None or nuevo_balance is None)):
                                    ...
                                    # --- VALIDADOR DE MEDICIÓN POKA-YOKE ---
                                    limite_excedido = False
                                    try:
                                        limite_num = float(limite_raw)
                                        if nuevo_valor_final > limite_num:
                                            limite_excedido = True
                                    except ValueError:
                                        pass

                                    if limite_excedido:
                                        medida_str = f"{nuevo_valor_final:.2f} s" if es_ion else f"{nuevo_valor_final:.2E} Ω"
                                        lim_str = f"{limite_num:.2f} s" if es_ion else f"{limite_num:.2E} Ω"
                                        st.error(f"🚨 **LEYENDA DE RECHAZO:** La medición ingresada ({medida_str}) **EXCEDE** el límite permitido ({lim_str}). El equipo NO aprueba la verificación. Revise los datos capturados o inhabilite el equipo desde la sección de Bajas.")
                                    else:
